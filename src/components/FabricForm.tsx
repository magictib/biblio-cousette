...
export default function FabricForm({ onSubmit, onCancel, initialValues }: FabricFormProps) {
  ...
  const [width,        setWidth]        = useState<string>(iv ? String(iv.width) : '140');
  const [lengthMeters, setLengthMeters] = useState<string>(iv && !iv.isScrap ? String(iv.length) : '2');
  const [scrapWidthCm, setScrapWidthCm] = useState<string>(iv?.isScrap ? String(iv.width)           : '');
  const [scrapLenCm,   setScrapLenCm]   = useState<string>(iv?.isScrap ? String(Math.round(iv.length * 100)) : '');

  const [isScrap,       setIsScrap]       = useState(iv?.isScrap        ?? false);
  const [estimatedArea, setEstimatedArea] = useState<string>(iv?.estimatedArea ? String(iv.estimatedArea) : '');
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div style={{ ...row2, marginBottom: '14px' }}>
        ...
        <div style={{ gridColumn: '1 / -1' }}>
          <label className="field-label">
            Couleur
            {detectingColor && <span style={{ marginLeft: '8px', fontSize: '0.72rem', color: 'var(--mauve)', fontStyle: 'italic' }}>⏳ Détection…</span>}
            {colorName && !detectingColor && <span style={{ marginLeft: '8px', fontSize: '0.72rem', color: 'var(--brun-mid)', fontStyle: 'italic' }}>— {colorName} (détecté)</span>}
          </label>
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            ...
            <input className="field-input" type="text" value={color}
              onChange={e => { setColor(e.target.value); setColorName(''); }} style={{ flex: 1 }} placeholder="#RRGGBB" />
            {photos.length > 0 && !detectingColor && (
              <button type="button" onClick={() => detectColor(photos[0])}
                style={{ padding: '6px 12px', borderRadius: '6px', border: '1.5px solid var(--mauve-light)', backgroundColor: 'var(--mauve-pale)', color: 'var(--mauve)', cursor: 'pointer', fontSize: '0.75rem', fontFamily: 'Georgia, serif', whiteSpace: 'nowrap' }}>
                🎨 Détecter
              </button>
            )}
          </div>
        ...
        <div>
          ...
          <input className="field-input" value={pattern} onChange={e => setPattern(e.target.value)} placeholder="Rayures, fleurs…" />
        </div>
      </div>

      {/* Toggle chute */}
      <button type="button" onClick={() => setIsScrap(v => !v)}
        style={{
        ...
        <span style={{ display: 'inline-block', width: '44px', height: '24px', borderRadius: '12px', backgroundColor: isScrap ? 'var(--mauve)' : '#ccc', position: 'relative', flexShrink: 0, transition: 'background-color 0.2s' }}>
          <span style={{ position: 'absolute', top: '3px', left: isScrap ? '23px' : '3px', width: '18px', height: '18px', borderRadius: '50%', backgroundColor: 'white', transition: 'left 0.2s', boxShadow: '0 1px 3px rgba(0,0,0,.2)' }}/>
        </span>
        <span>
          <span style={{ display: 'block', fontFamily: 'Georgia, serif', fontWeight: 'bold', color: isScrap ? 'var(--mauve)' : 'var(--brun-mid)', fontSize: '0.9rem' }}>✂️ Chute de tissu</span>
          <span style={{ display: 'block', fontSize: '0.75rem', color: 'var(--brun-mid)', opacity: 0.8 }}>Morceau irrégulier sans métrage précis</span>
        </span>
      </button>
      ...
      {!isScrap ? (
        <div style={{ marginBottom: '16px' }}>
          ...
          <div style={row2}>
            <div>
              <label style={{ fontSize: '0.75rem', color: 'var(--brun-mid)' }}>Largeur de laize ({unit === 'cm' ? 'cm' : 'pouces'})</label>
              <input className="field-input" type="number" min="1" step="1" required value={width} onChange={e => setWidth(e.target.value)} placeholder={unit === 'cm' ? '140' : '55'} />
            </div>
            <div>
              <label style={{ fontSize: '0.75rem', color: 'var(--brun-mid)' }}>Métrage (mètres)</label>
              <input className="field-input" type="number" min="0.1" step="0.1" required value={lengthMeters} onChange={e => setLengthMeters(e.target.value)} placeholder="2.5" />
            </div>
          </div>
        </div>
      ) : (
        <div style={{ marginBottom: '16px' }}>
          ...
          <div style={row2}>
            <div>
              ...
              <input className="field-input" type="number" min="1" step="1" value={scrapWidthCm} onChange={e => setScrapWidthCm(e.target.value)} placeholder="30" />
            </div>
            <div>
              <label style={{ fontSize: '0.75rem', color: 'var(--brun-mid)' }}>Longueur (cm)</label>
              <input className="field-input" type="number" min="1" step="1" value={scrapLenCm} onChange={e => setScrapLenCm(e.target.value)} placeholder="50" />
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ marginBottom: '16px' }}>
        <label className="field-label">
          ...
          {photos.length === 0 && <span style={{ marginLeft: '8px', fontSize: '0.72rem', color: 'var(--brun-mid)', fontStyle: 'italic', fontWeight: 'normal' }}>La couleur sera détectée automatiquement</span>}
        </label>
        {photos.length > 0 && (
            ...
            {photos.map((p, idx) => (
              <div key={idx} style={{ position: 'relative', width: '72px', height: '72px' }}>
                ...
                <button type="button" onClick={() => setPhotos(pr => pr.filter((_, i) => i !== idx))}
                  style={{ position: 'absolute', top: '-7px', right: '-7px', width: '20px', height: '20px', borderRadius: '50%', backgroundColor: 'var(--mauve)', color: 'white', border: 'none', cursor: 'pointer', fontSize: '0.65rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
              </div>
            ))}
          </div>
        )}
        <input type="file" accept="image/*" multiple ref={fileInputRef} onChange={handlePhotoUpload} style={{ display: 'none' }} />
        <button type="button" onClick={() => fileInputRef.current?.click()}
          style={{ padding: '8px 16px', border: '1.5px dashed var(--mauve-light)', borderRadius: '7px', backgroundColor: 'var(--linen)', color: 'var(--mauve)', cursor: 'pointer', fontFamily: 'Georgia, serif', fontSize: '0.85rem', display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
      ...
      {isScrap && photos.length > 0 && (
        <div style={{ padding: '14px 16px', marginBottom: '16px', backgroundColor: 'var(--linen)', border: '1.5px solid var(--mauve-pale)', borderRadius: '8px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
            <button type="button" onClick={estimateFromPhoto} disabled={estimating} className="btn-couture" style={{ fontSize: '0.85rem', opacity: estimating ? 0.7 : 1 }}>
            ...
            {estimatedArea && !estimating && (
              <span style={{ color: 'var(--sage)', fontFamily: 'Georgia, serif', fontWeight: 'bold', fontSize: '0.95rem' }}>
                ...
                <span style={{ fontSize: '0.75rem', color: 'var(--brun-mid)', fontWeight: 'normal', marginLeft: '6px' }}>({(parseInt(estimatedArea) / 10000).toFixed(3)} m²)</span>
              </span>
            )}
          </div>
          {estimateMsg && <p style={{ fontSize: '0.78rem', color: 'var(--brun-mid)', fontStyle: 'italic', margin: '8px 0 0' }}>{estimateMsg}</p>}
          <div style={{ marginTop: '10px' }}>
            <label style={{ fontSize: '0.75rem', color: 'var(--brun-mid)' }}>Surface estimée (cm²) — modifiable</label>
            <input className="field-input" type="number" min="1" value={estimatedArea} onChange={e => setEstimatedArea(e.target.value)} placeholder="Surface en cm²" style={{ marginTop: '4px' }} />
          </div>
        </div>
      )}
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <textarea className="field-input" value={notes} onChange={e => setNotes(e.target.value)}
          rows={3} style={{ resize: 'vertical' }} placeholder="Origine, lavage, remarques…" />
      </div>

      <div style={{ display: 'flex', gap: '10px' }}>
        <button type="submit" className="btn-sage" style={{ flex: 1, justifyContent: 'center' }}>
          {isEdit ? '✓ Enregistrer les modifications' : '＋ Ajouter le tissu'}
        </button>
        {onCancel && (
