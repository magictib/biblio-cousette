...
import { loadSharedCreationsDB, saveSharedCreationDB, deleteSharedCreationDB, type SharedCreation } from '@/lib/db';
...
interface FormState {
  name: string; status: Creation['status'];
  fabricName: string; patternName: string;
  tips: string; difficulties: string; notes: string;
  startedAt: string; finishedAt: string;
  photos: string[];
}

const emptyForm = (): FormState => ({
  name: '', status: 'en_cours', fabricName: '', patternName: '',
  tips: '', difficulties: '', notes: '',
  startedAt: new Date().toISOString().slice(0, 10), finishedAt: '', photos: [],
});

export default function CreationShare({ uid, username }: Props) {
  const [creations,  setCreations]  = useState<SharedCreation[]>([]);
  const [showForm,   setShowForm]   = useState(false);
  const [editing,    setEditing]    = useState<SharedCreation | null>(null);
  ...
  useEffect(() => {
    loadSharedCreationsDB().then(setCreations).catch(console.error);
  }, []);

  const openAdd = () => {
    setEditing(null); setForm(emptyForm()); setPhotoError(''); setShowForm(true);
  };

  const openEdit = (c: SharedCreation) => {
    if (c.userId !== uid) return;
    setEditing(c);
    setForm({
      name: c.name, status: c.status, fabricName: c.fabricName ?? '',
      patternName: c.patternName ?? '', tips: c.tips ?? '', difficulties: c.difficulties ?? '',
      notes: c.notes ?? '', startedAt: c.startedAt, finishedAt: c.finishedAt ?? '',
      photos: c.photos,
    });
    setPhotoError(''); setShowForm(true);
  };

  const cancelForm = () => { setShowForm(false); setEditing(null); setSaveError(''); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const saved = await saveSharedCreationDB(uid, username, payload);
      if (editing) {
        setCreations(prev => prev.map(c => c.id === editing.id ? saved : c));
      } else {
  ...
  const deleteCreation = async (id: string) => {
    try {
      await deleteSharedCreationDB(id);
    ...
    <div style={{ padding: '28px 28px 32px' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: 'var(--brun-mid)', fontSize: '0.85rem', margin: 0, fontStyle: 'italic' }}>
          Partagé entre tous les profils — tissus, patrons, astuces et difficultés
        </p>
      </div>
      ...
      {!showForm && (
        <div style={{ marginBottom: '24px' }}>
          <button onClick={openAdd} className="btn-sage">＋ Partager une création</button>
        </div>
      )}
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px', position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
              <div style={{ flex: '1 1 200px' }}>
                ...
                <input className="field-input" value={form.fabricName}
                  onChange={e => setF('fabricName', e.target.value)} placeholder="Nom ou description" />
              </div>
              <div style={{ flex: '1 1 200px' }}>
                <label className="field-label">Patron utilisé</label>
                <input className="field-input" value={form.patternName}
                  onChange={e => setF('patternName', e.target.value)} placeholder="Nom du patron" />
              </div>
            </div>
                ...
                {form.photos.map((src, i) => (
                  <div key={i} style={{ position: 'relative', width: '80px', height: '80px' }}>
                    ...
                    <button type="button" onClick={() => removePhoto(i)}
                      style={{ position: 'absolute', top: '-6px', right: '-6px', width: '20px', height: '20px', borderRadius: '50%', backgroundColor: '#943030', color: 'white', border: 'none', cursor: 'pointer', fontSize: '0.75rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      ✕
                    </button>
                  </div>
                ))}
            ...
            <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
              <div style={{ flex: '1 1 240px' }}>
                ...
                <textarea className="field-input" rows={3} value={form.tips}
                  ...
                  placeholder="Ce que j'ai appris…" style={{ resize: 'vertical' }} />
              </div>
              <div style={{ flex: '1 1 240px' }}>
                ...
                <textarea className="field-input" rows={3} value={form.difficulties}
                  ...
                  placeholder="Points délicats…" style={{ resize: 'vertical' }} />
              </div>
            </div>
            ...
            <div>
              ...
              <textarea className="field-input" rows={2} value={form.notes}
                ...
                placeholder="Impressions générales…" style={{ resize: 'vertical' }} />
            </div>

            <div style={{ display: 'flex', gap: '10px' }}>
              <button type="submit" className="btn-couture" disabled={saving}>
                {saving ? '⏳ Publication…' : editing ? '✓ Enregistrer' : '＋ Publier la création'}
              </button>
      ...
      {creations.length === 0 && !showForm ? (
        <div style={{ textAlign: 'center', padding: '48px 24px', color: 'var(--brun-mid)', fontStyle: 'italic', fontFamily: 'Georgia, serif' }}>
          ...
          <p>Aucune création partagée pour l&apos;instant.<br/>Soyez la première à en ajouter une !</p>
        </div>
      ) : (
            ...
            return (
              <div key={c.id} style={{
                ...
                <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--mauve-pale)', display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                  ...
                  {/* Badge auteur */}
                  <span style={{ fontSize: '0.72rem', color: 'var(--mauve)', fontFamily: 'Georgia, serif', fontStyle: 'italic' }}>
                    🧶 {c.username}
                  </span>
                  ...
                  <div style={{ flex: 1, minWidth: '220px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    {(c.fabricName || c.patternName) && (
                      <div style={{ backgroundColor: 'var(--linen)', border: '1px solid var(--mauve-pale)', borderRadius: '6px', padding: '10px 14px', fontSize: '0.82rem' }}>
                        {c.fabricName && <div style={{ display: 'flex', gap: '8px', marginBottom: c.patternName ? '4px' : 0 }}><span style={{ color: 'var(--brun-mid)' }}>🧵 Tissu :</span><span style={{ color: 'var(--brun)', fontWeight: 600 }}>{c.fabricName}</span></div>}
                        {c.patternName && <div style={{ display: 'flex', gap: '8px' }}><span style={{ color: 'var(--brun-mid)' }}>📋 Patron :</span><span style={{ color: 'var(--brun)', fontWeight: 600 }}>{c.patternName}</span></div>}
                      </div>
                    )}
                    {c.tips && <div><p style={{ fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--mauve)', fontFamily: 'Georgia, serif', margin: '0 0 4px' }}>💡 Astuces</p><p style={{ fontSize: '0.82rem', color: 'var(--brun)', margin: 0, lineHeight: 1.55 }}>{c.tips}</p></div>}
                    {c.difficulties && <div><p style={{ fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.06em', color: '#7A3030', fontFamily: 'Georgia, serif', margin: '0 0 4px' }}>⚠️ Difficultés</p><p style={{ fontSize: '0.82rem', color: 'var(--brun)', margin: 0, lineHeight: 1.55 }}>{c.difficulties}</p></div>}
                    {c.notes && <p style={{ fontSize: '0.82rem', color: 'var(--brun-mid)', fontStyle: 'italic', margin: 0, lineHeight: 1.5 }}>{c.notes}</p>}
                  </div>
                </div>
