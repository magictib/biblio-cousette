...
export default function CoutureApp() {
  ...
  // FORMULAIRE MIS À JOUR AVEC COULEUR
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.type === 'fabric') {
      await addDoc(collection(db, "fabrics"), {
        ...
        image: formData.imageUrl || "" // Vide si pas de photo
      });
    } else {
  ...
  return (
    <div className="min-h-screen relative bg-stone-50 text-stone-900 font-sans overflow-x-hidden">
      ...
      <div className="relative z-10 pb-24">
        <header className="bg-white/70 backdrop-blur-md p-6 sticky top-0 z-20 flex justify-between items-center border-b border-stone-200 shadow-sm">
          <div className="flex items-center gap-2">
            <div className="bg-emerald-700 p-1.5 rounded-lg text-white shadow-inner">
                <Scissors size={18} />
            </div>
              ...
              {fabrics.map(fabric => (
                <motion.div initial={{opacity:0, scale:0.95}} animate={{opacity:1, scale:1}} key={fabric.id} className="bg-white/90 backdrop-blur-sm rounded-[1.5rem] border border-stone-200 overflow-hidden shadow-sm flex flex-col h-full">
                  <div className="h-44 bg-stone-100 flex items-center justify-center relative overflow-hidden group">
                    ...
                    {/* Badge Couleur discret sur la photo */}
                  ...
                  <div className="p-4 flex-grow flex flex-col justify-between">
                    ...
                    <p className="text-[10px] text-emerald-700 font-bold tracking-widest">
          ...
          {view === 'projects' && (
            <div className="space-y-4">
              {projects.map(project => (
                <div key={project.id} className="bg-white/80 backdrop-blur-sm p-6 rounded-[2rem] border border-stone-200 shadow-sm">
                  ...
                  <div className="w-full bg-stone-200 h-2.5 rounded-full mb-6 overflow-hidden shadow-inner">
                    <div className="bg-emerald-500 h-full transition-all duration-700" style={{width: `${project.progress}%`}}></div>
                  </div>
                  <button onClick={() => updateProjectStep(project.id, project.currentStep)} className="w-full py-4 bg-stone-900 text-white rounded-2xl font-bold text-xs tracking-widest flex items-center justify-center gap-2 active:scale-95 transition shadow-lg">
            ...
            <div className="space-y-6 text-center pt-4">
               <div className="relative aspect-[3/4] bg-stone-900 rounded-[2.5rem] overflow-hidden border-8 border-white shadow-2xl mx-4">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-emerald-500/20">
                  ...
                  <p className="text-[10px] uppercase tracking-[0.4em] mt-6 font-black">Scanner de métrage</p>
                </div>
              </div>
              <p className="text-[10px] text-stone-500 uppercase tracking-[0.2em] font-bold italic opacity-60">L'IA de calcul de surface est en développement</p>
            </div>
          )}
        </main>

        {/* Barre de navigation */}
        <nav className="fixed bottom-6 left-4 right-4 bg-white/80 backdrop-blur-xl border border-stone-200 h-20 rounded-full shadow-2xl flex justify-around items-center px-4 z-40 shadow-emerald-900/10">
        ...
        {/* Modal d'ajout avec Champ COULEUR */}
        <AnimatePresence>
          {showModal && (
            <div className="fixed inset-0 bg-stone-900/40 backdrop-blur-md z-50 flex items-end sm:items-center justify-center p-4">
              <motion.div initial={{y:300}} animate={{y:0}} exit={{y:300}} className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl relative">
                <div className="flex justify-between items-center mb-6 border-b border-stone-100 pb-4">
                  <h2 className="text-xl font-black uppercase tracking-tighter text-stone-800">Ajouter</h2>
                  <button onClick={() => setShowModal(false)} className="p-2 bg-stone-100 rounded-full text-stone-400"><X size={20}/></button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {formData.type === 'fabric' && (
                    <>
                      ...
                      <div className="relative h-40 w-full bg-stone-50 rounded-3xl border-2 border-dashed border-stone-200 flex flex-col items-center justify-center overflow-hidden mb-4 group active:bg-stone-100 transition shadow-inner">
                        {uploading ? (
                          <div className="flex flex-col items-center gap-2">
                             <Loader2 className="animate-spin text-emerald-600" />
                             <span className="text-[8px] font-bold uppercase text-stone-400">Envoi en cours...</span>
                          </div>
                        ) : formData.imageUrl ? (
                          ...
                          <div className="text-center">
                            ...
                            <span className="text-[10px] uppercase font-black text-stone-400 tracking-tighter">Photo du tissu (Optionnel)</span>
                          </div>
                        )}
                      ...
                      <div className="relative">
                        ...
                        <input placeholder="COULEUR (EX: BLEU CANARD)" className="w-full p-4 pl-12 bg-stone-100 rounded-2xl outline-none text-[10px] font-bold uppercase tracking-widest focus:ring-2 ring-emerald-500 transition-all" value={formData.color} onChange={e => setFormData({...formData, color: e.target.value})} />
                      </div>
                    </>
                  )}

                  <input required placeholder="NOM DE L'ÉLÉMENT" className="w-full p-4 bg-stone-100 rounded-2xl outline-none text-[10px] font-bold uppercase tracking-widest placeholder:text-stone-300 focus:ring-2 ring-emerald-500 transition-all" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                  
                  {formData.type === 'fabric' && (
                    <div className="flex gap-4">
                      <input placeholder="LONG. (M)" className="w-1/2 p-4 bg-stone-100 rounded-2xl outline-none text-[10px] font-bold tracking-widest focus:ring-2 ring-emerald-500 transition-all" value={formData.length} onChange={e => setFormData({...formData, length: e.target.value})} />
                      <input placeholder="LAIZE (CM)" className="w-1/2 p-4 bg-stone-100 rounded-2xl outline-none text-[10px] font-bold tracking-widest focus:ring-2 ring-emerald-500 transition-all" value={formData.width} onChange={e => setFormData({...formData, width: e.target.value})} />
                    </div>
                  )}
