    ...
    <div className="min-h-screen bg-stone-50 pb-24 text-stone-900 font-sans">
      <header className="bg-white/80 backdrop-blur-md p-6 sticky top-0 z-20 flex justify-between items-center border-b border-stone-200">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold tracking-tight text-emerald-800 uppercase">Biblio Cousette</h1>
        </div>
      ...
      <main className="p-4 max-w-xl mx-auto">
        {view === 'inventory' && (
          <div className="grid grid-cols-2 gap-4">
            ...
            {fabrics.length === 0 && <p className="col-span-2 text-center text-stone-400 mt-10 italic text-sm">Votre tissuthèque est vide.</p>}
          </div>
        )}
          ...
          <div className="space-y-4">
            {projects.map(project => (
              <div key={project.id} className="bg-white p-5 rounded-3xl border border-stone-200 shadow-sm">
                ...
                <button onClick={() => updateProjectStep(project.id, project.currentStep)} className="w-full py-3 bg-stone-900 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 active:scale-95 transition">
      ...
      <nav className="fixed bottom-6 left-4 right-4 bg-white/90 backdrop-blur-lg border border-stone-200 h-20 rounded-full shadow-2xl flex justify-around items-center px-4 z-40">
        ...
        <button onClick={() => setView('scan')} className="bg-emerald-600 text-white p-5 rounded-full -mt-12 shadow-xl shadow-emerald-200 active:scale-90 transition"><Camera size={28} /></button>
